...
<title id="p1-t">Measure for 30 minutes, route for 5, classify for 5, compose for 10, decide with the table for 5, then review quarterly.</title>
...
<text class="dg-s" x="360" y="140" text-anchor="middle">quarterly re-measure</text>
</svg>
</div>
...
Run the [minimum measurement](ch:ten-methods#before-any-method-measure): the four routing numbers are about 30 minutes of its hour.
...
> The most common source of wrong context decisions in this research: reasoning from intuition about which segment was large.
...
| Sessions often exceed the window | History growth | **[M-8](ch:ten-methods#m-8-semantic-boundary-compaction) and [M-6](ch:ten-methods#m-6-reversible-offload)** | M-7, M-10 |
| [[Read-utilisation]] under 5% | Retrieval precision | **[M-4 structural retrieval](ch:ten-methods#m-4-structural-retrieval)** | M-3 |
| Failures are starvation (decisive file never read) | Retrieval recall | **[M-4](ch:ten-methods#m-4-structural-retrieval) first, then [M-3](ch:ten-methods#m-3-just-in-time-retrieval)** | — |
...
<rect class="dg-box--muted" x="10" y="340" width="700" height="46" rx="8"/><text class="dg-t" x="360" y="368" text-anchor="middle">None of these? Four more constraints sit in the routing table above.</text>
...
The mechanism behind each rule is chapter 3's [map](ch:ten-methods#the-ten-on-one-page).
...
|---|---|---|---|
| Context at 70%, task half done | Reset with a handoff | Compact and continue | Compaction is lossy *and* breaks the cache; a [reset costs about 500 tokens](ch:ten-methods#m-10-session-lifecycle) |
