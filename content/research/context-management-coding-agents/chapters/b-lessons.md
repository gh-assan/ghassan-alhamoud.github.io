...
|---|---|---|---|
| L1 | **The context window is an attention budget, not a container.** | Adding tokens can redistribute useful signal; Chroma's curves show non-uniform, model- and task-dependent degradation rather than a universal token-level law [S]. | a controlled study finds task performance monotonically non-decreasing in irrelevant added context. |
| L2 | **Adding correct information can reduce performance.** | Correctness does not guarantee salience; controlled studies show irrelevant context can reduce retrieval reliability [S]. | adding verified-correct, task-irrelevant context reliably improves solve rates. |
| L3 | **Degradation can begin before the window fills.** | In Chroma's tested set, performance generally fell as input grew, but not at every increment or for every model [S]. | a comparable controlled study finds flat performance across the tested range. |
| L4 | **Position can change retrieval reliability.** | The cited tasks found a U-shaped position effect, with start/end better than middle; there is no fixed two-slot or 50% rule [S]. | mid-context placement performs equally to end-context placement in a comparable task. |
| L5 | **Early rules can become harder to retrieve as context grows.** | This is a model- and harness-dependent transfer from position findings; test it on the actual instruction path [D]. | rule-violation rate is flat with respect to session depth at constant instruction-file content. |
...
| L7 | **Effective context length is a workload property, not an advertised constant.** | The length at which performance remains acceptable depends on the model and task; measure it instead of inheriting the provider's limit [D]. | repeated workload tests remain equally reliable near the nominal maximum and at shorter lengths. |
| L8 | **Coding agents are a special case because verification is executable.** | Tests and compilers provide partial, executable evidence, so some lost facts surface as failing checks rather than plausible falsehoods. | context-management errors in repos with strong test suites produce as many silent wrong answers as in repos without. |
...
| # | Lesson | Why it is true | Falsified if |
|---|---|---|---|
| L9 | **In one verbose tool-use benchmark, input dominated token volume.** | 99.75–99.87% across four GPT-5 configurations averaged over five runs on a 50-task Dynamics 365 hotel-expense benchmark using verbose MCP responses [S]; this is not a cost share. | Your own workload has a materially different input share, or output pricing/cache status changes which intervention reduces priced cost. |
...
| L11 | **A large stable prefix can cost less than a small churning one.** | In chapter 10's illustrative input-only price model, a 24% input-token cut produced a 6.9× increase in estimated input cost when the prefix became dynamic; output charges are excluded. | Your provider's cache rates, output pricing or request pattern reverse the measured total-cost result. |
...
| # | Lesson | Why it is true | Falsified if |
|---|---|---|---|
| L18 | **The programme prefers reversibility over compression ratio.** | A recoverable reduction preserves a recovery path; an irrecoverable reduction does not. | recall is never invoked and stub overhead exceeds summarisation savings. |
...
| L34 | **Retrieval recall and task outcome anticorrelate past a point.** | more retrieved context raises recall and lowers density; focused prompts retain the answer-bearing material while removing surrounding context [S]. | solve rate rises monotonically with retrieved-token count. |
...
| L42 | **Pre-load pointers, never content.** | pointers help the agent decide where to look at ~1% of the cost; content substitutes for looking and dilutes. A Sourcegraph author reports 5K-targeted retrieval outperforming a 100K summary on the same task [P] (see [post](https://sourcegraph.com/blog/context-engineering)), but names no task, model or comparison method; treat this vendor/practitioner observation as a prompt to test, not benchmark evidence. | a content pre-load outperforms a pointer seed at matched task difficulty. |
...
| # | Lesson | Why it is true | Falsified if |
|---|---|---|---|
| L45 | **Tool surfaces can create both token cost and selection load.** | Definitions consume prefix tokens; larger pools can complicate tool choice, but the cited evaluations do not isolate these effects. | a controlled comparison finds no selection or task-success change as candidate pools grow while prompt size and harness are held constant. |
| L46 | **Tool-pool size is a workload-specific variable, not a universal ceiling.** | RAG-MCP's 43.13% vs 13.62% is a method comparison; its separate distractor-pool stress test degrades non-monotonically at large sizes [S]. A small vendor Dog API demonstration reports 19/20 at 20 tools, 3/4 at 40, and frequent errors at 107 [P]. | replicated representative evaluations find no meaningful task-success or wrong-tool change as candidate pools expand under controlled prompt and harness conditions. |
