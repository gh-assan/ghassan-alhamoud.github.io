...
> Three MCP servers: GitHub (42K), a database server (11K) and a browser server (9K): 62K of definitions, 31% of a 200K window, before any work. Over 20 sessions, 9 tools were called, all from GitHub, and 6 of those duplicated the `gh` command already available. The team drops the browser server (never used), drops the database server (a read-only `psql` command replaces it), and switches GitHub to deferred definitions. New prefix contribution: **about 4K. Recovered: 58K tokens, 29% of the window.** Task success is unchanged over the next 20 sessions; median session cost falls 34%.
